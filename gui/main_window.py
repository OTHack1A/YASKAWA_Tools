    ...
    def _on_write(self):
        ...
        if ok:
            ...
            try:
                ...
                folder = reveal_in_explorer(var_path)
...
&nbsp;&nbsp;<i>I nomi vanno scritti partendo da un template generato dal tool stesso (voce Template): va compilato e poi ri-esportato da qui. Solo così, importando i file nel robot YASKAWA, non si verificano problemi.</i><br>
...
&nbsp;&nbsp;<i>Names must start from a template generated by the tool itself (Template entry): fill it in, then re-export it here. This way, importing the files into the YASKAWA robot causes no problems.</i><br>
...
&nbsp;&nbsp;<i>Les noms doivent partir d'un modèle généré par l'outil lui-même (entrée Template) : remplissez-le, puis ré-exportez-le ici. Ainsi, l'importation des fichiers dans le robot YASKAWA ne pose aucun problème.</i><br>
...
&nbsp;&nbsp;<i>Los nombres deben partir de una plantilla generada por la propia herramienta (entrada Template): se rellena y luego se vuelve a exportar aquí. Así, al importar los archivos en el robot YASKAWA, no hay problemas.</i><br>
...
&nbsp;&nbsp;<i>Die Namen müssen von einer vom Tool selbst erzeugten Vorlage ausgehen (Eintrag Template): ausfüllen und dann hier erneut exportieren. Nur so treten beim Import der Dateien in den YASKAWA-Roboter keine Probleme auf.</i><br>
...
&nbsp;&nbsp;<i>Назви потрібно створювати на основі шаблону, згенерованого самим інструментом (пункт Template): заповніть його, а потім повторно експортуйте тут. Лише так імпорт файлів у робот YASKAWA відбувається без проблем.</i><br>
...
class MainWindow(QMainWindow):
    ...
    def _reveal_in_explorer(self, path):
        ...
        elsewhere the folder itself is opened. Used after every file
        generation so the user always lands on the saved file.
        """
        try:
            from docs.fsutil import reveal_in_explorer
            folder = reveal_in_explorer(path)
            logger.info("log_open_folder", folder)
        except Exception as exc:
            logger.warning("log_error_generic", str(exc))
    ...
    def on_template(self):
        ...
        # Apri sempre la cartella di destinazione dove è stato salvato il file
    ...
    def _run_dat_generation(self, excel_path):
        ...
        try:
            ...
            logger.error("log_error_generic", str(exc))
            return
        self._reveal_in_explorer(out_folder)
    ...
    def _run_nomi_generation(self, tmp_path):
        ...
        try:
            ...
            logger.error("log_error_generic", str(exc))
            return
        # Apri sempre la cartella di destinazione dei file generati
        self._reveal_in_explorer(out_folder)
    ...
    def on_motion_drive(self):
        ...
        # Close any panel open underneath and take over the full work area
        # (without _hide_welcome the welcome screen stayed visible and the
        # drive view was squeezed into half the page).
        self._close_all_panels()
        self._hide_welcome()

        self._progress_begin()
        try:
            self._progress_update(20)
            from docs.drive import parse_ydwiproj
            info, params_by_cat = parse_ydwiproj(file_path)
            self._progress_update(60)
